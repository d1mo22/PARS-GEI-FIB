(define (domain pddl)
    ...
    (:predicates
        ...
        ; d1 is the day before d2
        (before ?d1 - day ?d2 - day)
    )

    (:functions
        (minutesWatched ?d - day) ;How many minutes we have watched in the day d
        (duration ?cont - content) ;Content duration in minutes
    )
    ...
    (:action watch
        ...
        :precondition (and
            (toWatch ?cont)
            (not (watch ?cont))
            (<= (+ (minutesWatched ?d) (duration ?cont)) 200)
        )
        :effect (when
            (not
                (exists
            ...
            (and (watchedDay ?cont ?d) (watch ?cont) (increase (minutesWatched ?d) (duration ?cont)))
        )
    )
)