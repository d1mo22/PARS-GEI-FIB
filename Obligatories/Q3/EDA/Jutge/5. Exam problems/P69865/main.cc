...
int dfs_k(const PI &start, VVI& visitats) {
  stack<PI> q;
  int suma = 0;
  ...
  while (!q.empty() and (tresors != 0)) {
    ...
    if (!visitats[x][y]) {
      ...
      for (int i = 0; i < 8; ++i) {
        ...
        if (pos_ok(xx, yy) and !visitats[xx][yy]) {
          q.push({xx, yy});
        }
      }
    }
...
int dfs_b(const PI &start, VVI& visitats) {
  stack<PI> q;
  int suma = 0;
...
int main() {
  while (cin >> n >> m) {
    ...
    for (int i = 0; i < k; ++i) {
      ...
      punts += dfs_k(pos, visitats);
    }
    for (int i = 0; i < b; ++i) {
      PI pos = alfils[i];
      VVI visitats(n, VI(m, false));
      punts += dfs_b(pos,visitats);
    }
    cout << punts << endl;
  }
}
