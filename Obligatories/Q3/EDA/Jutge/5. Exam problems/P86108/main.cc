...
int back(Mapa &sol, int n, int m, int i, int j, int num) {
  ...
  if (j >= 2 and sol[i][j - 1] == 'O' and sol[i][j - 2] == 'L')
    ++num_lol;
  // comprovem si s'ha format un LOL diagonal esq-dre
  if (i >= 2 and j >= 2 and sol[i - 1][j - 1] == 'O' and
      sol[i - 2][j - 2] == 'L')
    ++num_lol;
  // comprovem si s0ha format un LOL en vertical
  if (i >= 2 and sol[i - 1][j] == 'O' and sol[i - 2][j] == 'L')
    ++num_lol;
  // comprovem si s'ha format un LOL diagonal dre-esq
  if (i >= 2 and j < m - 1 and sol[i - 1][j + 1] == 'O' and
      sol[i - 2][j + 2] == 'L')
    ++num_lol;
