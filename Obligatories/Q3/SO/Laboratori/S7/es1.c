...
// Este codigo escribe todos los bytes que lee de la entrada std en la salida
// stda
...
int main() {
    ...
    sprintf(buffer,
            "Este programa escribe por la salida std todo lo que lee de la "
            "entrada std.\nSi no has redirigido la salida, lo que escribas en "
            "el teclado saldra por la pantalla\n");
    write(2, buffer, strlen(buffer));
