...
LL4GLWidget::LL4GLWidget(QWidget *parent)
...
void LL4GLWidget::mouseReleaseEvent(QMouseEvent *) { DoingInteractive = NONE; }

void LL4GLWidget::calculaCapsaModel(Model &p, float &escala,
                                    float profunditatDesitjada,
                                    glm::vec3 &centreBase) {
...
void LL4GLWidget::creaBuffersModels() {
  ...
  float profunditatDesitjada[NUM_MODELS] = {
      2, 1, 3}; // nota: PIPE té alçada = profunditat

  for (int i = 0; i < NUM_MODELS; i++) {
    // Calculem la capsa contenidora del model
    calculaCapsaModel(models[i], escalaModels[i], profunditatDesitjada[i],
                      centreBaseModels[i]);
...
void LL4GLWidget::creaBuffersTerra() {
  ...
  glm::vec3 posterra[6] = {
      glm::vec3(0.0, 0.0, 10.0),  glm::vec3(10.0, 0.0, 10.0),
      glm::vec3(0.0, 0.0, 0.0),   glm::vec3(0.0, 0.0, 0.0),
      glm::vec3(10.0, 0.0, 10.0), glm::vec3(10.0, 0.0, 0.0),
  };
  ...
  glm::vec3 matambterra[6] = {amb, amb, amb, amb, amb, amb};
  glm::vec3 matdiffterra[6] = {diff, diff, diff, diff, diff, diff};
  glm::vec3 matspecterra[6] = {spec, spec, spec, spec, spec, spec};
  float matshinterra[6] = {shin, shin, shin, shin, shin, shin};
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(matambterra), matambterra,
               GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(matdiffterra), matdiffterra,
               GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(matspecterra), matspecterra,
               GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(matshinterra), matshinterra,
               GL_STATIC_DRAW);
